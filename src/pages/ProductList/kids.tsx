...
export default function KidsPage() {
  ...
  useEffect(() => {
    ...
    const fetchKidsProducts = async () => {
      try {
        ...
        const kidsProducts: Product[] = res.data.products
          .filter((p: any) => p.category === "Kids") // Filter for "Kids"
          .map((p: any) => ({
            id: p.productId,
            title: p.name,
            subtitle: p.subtitle || "",
  ...
  if (loading) return <p className="text-center mt-10">Loading kids' products...</p>;
  ...
  return (
    <main>
      ...
      <section className="mx-auto max-w-8xl px-4 py-8 md:px-6">
        <div className="mb-4">
          <h1 className="text-2xl font-semibold text-gray-900">Kids' Collection</h1>
          <p className="text-sm text-gray-500">Trendy outfits and essentials for kids</p>
        </div>
