...
export default function WomenPage() {
  ...
  useEffect(() => {
    ...
    const fetchWomenProducts = async () => {
      try {
        ...
        const womenProducts: Product[] = res.data.products
          .filter((p: any) => p.category === "Women") // ✅ Filter for Women's category
          .map((p: any) => ({
            id: p.productId,
            title: p.name,
            subtitle: p.subtitle || "",
  ...
  if (loading) return <p className="text-center mt-10">Loading women’s products...</p>;
  ...
  return (
    <main>
      ...
      <section className="mx-auto max-w-8xl px-4 py-8 md:px-6">
        <div className="mb-4">
          <h1 className="text-2xl font-semibold text-gray-900">Women's Collection</h1>
          <p className="text-sm text-gray-500">Discover the latest trends for women</p>
        </div>
